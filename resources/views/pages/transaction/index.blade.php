                            ...
                            <table class="min-w-full bg-gray-700  rounded-lg shadow-sm">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider bg-gray-700">Menu Name</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider bg-gray-700">Quantity</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider bg-gray-700">Total Price</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider bg-gray-700">Date</th>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach ($transactions as $trans)
                                        <tr class="">
                                            <td class="px-6 py-4 text-sm text-white bg-gray-500 ">{{ $trans->cashier->name }}</td>
                                            ...
                                            <td class="px-6 py-4 text-sm bg-gray-500">
                                                <div class="flex space-x-2">
                                                    <a href="{{ route('transaction.edit', $trans->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600">Edit</a>
                                                    <form action="{{ route('transaction.destroy', $trans->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this item?');">
                                                        @csrf
                                                        @method('DELETE')
